    ...
    <main className="p-4 max-w-5xl mx-auto grid gap-3">
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          <input className="border rounded px-3 py-2 w-56" placeholder="Buscar…" value={q} onChange={e=>setQ(e.target.value)} />
          <select className="border rounded px-3 py-2" value={order} onChange={e=>setOrder(e.target.value as 'cat_name'|'name')}>
            <option value="cat_name">Categoría + Nombre</option>
            <option value="name">Nombre</option>
          </select>
          <button onClick={()=>setShowNew(true)} className="border rounded px-3 py-2">Nuevo</button>
          <button onClick={()=>setMostrarPapelera(v=>!v)} className="border rounded px-3 py-2">{mostrarPapelera?'Ver activos':'Papelera'}</button>
        </div>
      </div>
      <div className="grid gap-3">
        <div className="text-sm text-gray-600">Categorías: {categorias.map(c=> `${c.id}:${c.nombre}`).join(' • ')}</div>
        {productos.map(p => (
          <div key={p.id} className="border rounded p-3 flex items-center gap-3">
            ...
            <div className="flex-1 grid gap-1">
              <div className="flex items-center gap-2">
                <input className="border rounded px-2 py-1" defaultValue={p.nombre} onBlur={async(e)=>{
                ...
                <input className="border rounded px-2 py-1 w-28 text-right" defaultValue={(p.precioCents/100).toFixed(2)} onBlur={async(e)=>{
                ...
                <input title="Costo" className="border rounded px-2 py-1 w-28 text-right" defaultValue={((p.costoCents||0)/100).toFixed(2)} onBlur={async(e)=>{
                ...
                <select className="border rounded px-2 py-1" value={p.categoriaId} onChange={async(e)=>{
                ...
                {!mostrarPapelera ? (
                  <button
                  className="ml-auto text-red-600 border rounded px-2 py-1"
                  onClick={async ()=>{
      ...
      {editExtras && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded shadow-lg p-4 w-full max-w-lg grid gap-3">
            ...
            <div className="grid gap-2">
              ...
              {editExtras.rows.map((row, idx)=> {
                ...
                <div key={idx} className="flex items-center gap-2">
                  ...
                  />
                  <button className="text-red-600 border rounded px-2 py-1" onClick={()=>{
              ...
              <div>
                <button className="border rounded px-2 py-1" onClick={()=> setEditExtras(prev=> prev ? { ...prev, rows: [...prev.rows, { nombre:'', precio:'' }] } : prev)}>
                  + Agregar extra
                </button>
              </div>
            </div>
            <div className="flex gap-2 justify-end pt-2">
              <button className="border rounded px-3 py-2" onClick={()=>setEditExtras(null)}>Cancelar</button>
              {(()=>{ const v = validateRows(editExtras.rows); return (
              <button className="border rounded px-3 py-2 disabled:opacity-50" disabled={v.hasErrors} onClick={async()=>{
                ...
                if (!res.ok) {
                  ...
                  alert(j?.error || 'No se pudo guardar los extras. ¿Tienes permisos de administrador?')
                  return
                }
                setEditExtras(null)
                await load()
      ...
      {showNew && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center">
          <div className="bg-white rounded shadow-lg p-4 w-full max-w-md grid gap-3">
            <h2 className="text-lg font-semibold">Nuevo producto</h2>
            <label className="grid gap-1">
              <span className="text-sm">Nombre</span>
              <input className="border rounded px-3 py-2" value={newNombre} onChange={e=>setNewNombre(e.target.value)} />
            </label>
            <label className="grid gap-1">
              <span className="text-sm">Precio</span>
              <input className="border rounded px-3 py-2" placeholder="0.00" value={newPrecio} onChange={e=>setNewPrecio(e.target.value)} />
            </label>
            <label className="grid gap-1">
              <span className="text-sm">Costo</span>
              <input className="border rounded px-3 py-2" placeholder="0.00" value={newCosto} onChange={e=>setNewCosto(e.target.value)} />
            </label>
            <label className="grid gap-1">
              <span className="text-sm">Categoría</span>
              <select className="border rounded px-3 py-2" value={newCategoriaId ?? ''} onChange={e=>setNewCategoriaId(Number(e.target.value))}>
            ...
            <label className="grid gap-1">
              ...
              <input className="border rounded px-3 py-2" placeholder="ej. Lechuga, Tomate, Queso" value={newIngredientes} onChange={e=>setNewIngredientes(e.target.value)} />
            </label>
            <div className="flex gap-2 justify-end pt-2">
              <button className="border rounded px-3 py-2" onClick={()=>{ setShowNew(false); setNewNombre(''); setNewPrecio(''); setNewCosto(''); setNewCategoriaId(undefined); setNewFile(null); }}>Cancelar</button>
              <button className="border rounded px-3 py-2" onClick={crear}>Crear</button>
            </div>
          </div>
        </div>
      )}
