...
export default function ContabilidadDashboard({ defaultTab = 'financials' }: { defaultTab?: 'financials' | 'dgii' | 'ncf' }) {
  const [activeTab, setActiveTab] = useState<'financials' | 'dgii' | 'ncf'>(defaultTab)
  ...
  return (
    <AdminLayout
      title="Contabilidad Avanzada y DGII"
      actions={
        <div className="flex bg-slate-100 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-2xl p-1 gap-1 shadow-inner shadow-black/10 dark:shadow-black/40">
          <button
          ...
          >
            <BarChart3 size={14} /> Estados Financieros
          </button>
          <button
          ...
          >
            <FileSpreadsheet size={14} /> Envío DGII (606/7/8)
          </button>
          <button
          ...
          >
            <Settings size={14} /> Secuencias NCF
          </button>
        </div>
      }
    >
